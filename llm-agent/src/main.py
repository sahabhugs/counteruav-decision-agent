...
class DecideRequest(BaseModel):
    ...
    situation: SituationData = Field(..., description="态势信息")
...
async def lifespan(app: FastAPI):
    ...
    tools_registry.register(
        "run_topsis",
        run_topsis,
        "执行TOPSIS多属性威胁评估计算。参数: target_id(必需,目标ID)"
    )
    tools_registry.register(
        "check_devices",
        check_devices,
    ...
    tools_registry.register(
        "propose_rule",
        propose_rule,
        "向规则引擎提交新处置规则提案。参数: rule_text(必需,规则文本), reason(必需,提案原因), "
        "source_decision_id(必需,来源决策ID)"
    )
...
async def llm_decide(request: DecideRequest):
    ...
    if not rate_limiter.try_acquire(target_id):
        logger.warning(f"限流拒绝: task_id={task_id}, target_id={target_id}")
        raise HTTPException(
            status_code=429,
            detail=f"请求频率超限，请稍后重试。全局限制: {app_config.MAX_CALLS_PER_MINUTE}次/分钟，"
                   f"单目标限制: 3次/分钟，冷却时间: {app_config.COOLDOWN_SECONDS}秒",
        )
